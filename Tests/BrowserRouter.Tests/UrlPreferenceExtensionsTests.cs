    ...
    public class UrlPreferenceExtensionsTests
    {
        ...
        public void GetDomainAndPatternQuery()
        {
...

            {
                ...
                var (domain, pattern) =
                    domainPref.GetDomainAndPattern(new Uri("https://www.youtube.com/redirect?event=video_description&redir_token=QUFFLU&q=https%3A%2F%2Fwww.reddit.com"));

                Assert.That(domain, Is.EqualTo("www.youtube.com/redirect?event=video_description&redir_token=QUFFLU&q=https%3A%2F%2Fwww.reddit.com"));
                Assert.That(pattern, Is.EqualTo("^www\\.youtube\\.com/redirect\\?.*reddit\\.com.*$"));
                preferences.Add(domainPref);
            }

            Assert.That(preferences.TryGetPreference(new Uri("https://www.youtube.com/redirect?event=video_description&redir_token=QUFFLU&q=https%3A%2F%2Fwww.reddit.com"), out var pref1), Is.True);
